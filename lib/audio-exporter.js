...
// onProgress({ downloaded, totalBytes, percent, phase }) 在下载过程中按字节推送实时进度
export async function downloadAndExportTrack({ outputRoot, playlistName, artist, title, downloadUrl, cookie, onProgress }) {
  const { playlistDir, filePath } = resolvePlaylistOutputPath(outputRoot, playlistName, artist, title);
  await mkdir(playlistDir, { recursive: true });
