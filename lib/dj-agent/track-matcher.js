...
export function scoreCandidateSong(target, candidate) {
  ...
  let remixBonus = 0;
  const isWeakVersion = /\b(mashup|bootleg|acappella|a cappella)\b/i.test(`${targetTitle} ${effectiveTargetRemix}`);
  if (isWeakVersion) {
    // 网易云几乎不收录 Mashup/Bootleg/Acappella 版本, 匹配基础曲目是务实结果, 不罚不奖
    remixBonus = 0;
  } else if (effectiveTargetRemix) {
...
export async function matchSingleTrack(track, cookie = "") {
  ...
  // 缺少歌名的条目 (1001TL Acapella/纯人声行或解析残缺片段) 无法可靠匹配
  if (!track.title || !(track.title || "").trim()) {
    return {
      trackNumber: track.trackNumber,
      original: track,
      matched: false,
      reason: "条目缺少歌名 (Acapella/纯人声行), 无法可靠匹配",
    };
  }
  ...
  try {
    // 构造渐进式检索关键词序列 (从最精准版本名 -> 去 ft. 艺人 -> 核心歌名 -> 纯歌名)
    const queriesToTry = [];
    if (normalizedQuery) queriesToTry.push(normalizedQuery);
    if (baseQuery && baseQuery !== normalizedQuery) queriesToTry.push(baseQuery);

    const cleanTitle = (track.title || "").replace(/\([^)]*\)/g, "").replace(/\[[^\]]*\]/g, "").replace(/\s+/g, " ").trim();

    // 核心歌名（去掉括号）
    if (track.artist && cleanTitle) {
      const coreTitle = `${track.artist} ${cleanTitle}`.replace(/\s+/g, " ").trim();
      if (!queriesToTry.includes(coreTitle)) queriesToTry.push(coreTitle);

      // 去除 ft./feat./featuring 参与艺人后的查询 (网易云对 "ft." 查询支持极差, 常返回翻唱/伴奏)
      const mainArtist = track.artist.replace(/\s+(ft\.?|feat\.?|featuring|with)\b.*$/i, "").trim();
      if (mainArtist && mainArtist !== track.artist) {
        const noFtQuery = `${mainArtist} ${cleanTitle}`.replace(/\s+/g, " ").trim();
        if (!queriesToTry.includes(noFtQuery)) queriesToTry.push(noFtQuery);
      }
    }

    // 纯歌名兜底 (艺人写法差异大时, 由候选评分的艺人包含度把关)
    if (cleanTitle && !queriesToTry.some((q) => q === cleanTitle)) {
    ...
    for (const q of queriesToTry) {
      try {
        // 搜索走匿名通道 (不带 Cookie): A/B 实测带 Cookie 与匿名搜索结果完全一致,
        // 但带 Cookie 连续大批量搜索会触发账号级限流 (返回空结果)。
        // Cookie 仅用于后续 320k 直链检测。
        let searchRes = await fetchNetEaseApi("/cloudsearch/pc", {
          params: { s: q, type: 1, limit: 15, offset: 0 },
          cookie: "",
        });
        let currentCandidates = (searchRes?.result?.songs || []).map((s) => ({
          ...s,
          artists: s.ar || s.artists || [],
          album: s.al || s.album || {},
        }));

        // 空结果重试: 批量匹配时偶发限流 (返回空), 等待 600ms 后重试一次
        if (currentCandidates.length === 0) {
          ...
          currentCandidates = (searchRes?.result?.songs || []).map((s) => ({
            ...s,
            artists: s.ar || s.artists || [],
            album: s.al || s.album || {},
          }));
