...
`.trim(),
  ...
  async execute(params, context) {
    ...
    if (targetUrl) {
      ...
      // 如果文本中未包含多行曲目（例如用户仅输入了演出名称如 'Sub Focus @ Rampage Open Air'）
      if (!parsedSet.tracks || parsedSet.tracks.length < 2) {
        const cleanQuery = rawText.replace(/^(请|帮我|解析|下载|制作|生成|已锁定：|歌单|现场)/g, "").trim();
...
...`;
        ...
        try {
          ...
          parsedSet = parseTracklistText(llmRes.content, { filterUnreleased: true });
          parsedSet.title = cleanQuery.startsWith("Sub Focus") || cleanQuery.startsWith("Martin Garrix") || cleanQuery.includes("@") 
            ? cleanQuery 
            : `DJ Set: ${cleanQuery}`;
