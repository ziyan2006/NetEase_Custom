      ...
      if (onStream) {
        onStream({
          ...
          data: {
            ...
            message: `计算 Camelot 调性轮: 当前 ${normalized} -> 顺时针 ${compatible.plusOne} / 能量爆发 ${compatible.energyBoost}`,
          },
        });
      }
      harmonicContext = `
...
- 顺时针半音突变 (+7 Semitone Jump): ${compatible.semitonePlus}
- 逆时针半音突变 (-5 Semitone Jump): ${compatible.semitoneMinus}
- 相对大小调主根音同度 (+3): ${compatible.dominant}
`.trim();
    }
