...
export async function dispatchAgentWorkflow({
  ...
  const isSetSearchIntent = /(?:演出|现场|set|setlist|liveset)/i.test(text) && !urlMatch;
  if (isSetSearchIntent) {
    ...
    if (cleanedQuery.length >= 2) {
      ...
      const searchResult = await searchArtistRecentSets(cleanedQuery);
      if (searchResult.sets && searchResult.sets.length > 0) {
        const cardPayload = {
          title: `🎪 ${searchResult.artist} 现场演出推荐列表`,
          subtitle: `在 1001Tracklists 找到 ${searchResult.sets.length} 个代表性 Setlist，点击即可一键解析并生成歌单：`,
  ...
  if (urlMatch) {
    ...
    try {
      const parsedSet = await fetchAndParse1001TracklistUrl(targetUrl, { filterUnreleased: true });
      if (onStream) {
        onStream({
          type: "status",
  ...
  for (const [key, profile] of Object.entries(GENRE_PROFILES)) {
    if (
      ...
      if (lower.includes("榜") || lower.includes("热单") || lower.includes("推荐") || lower.includes("歌单") || lower.includes("top") || lower.includes("chart")) {
        matchedGenre = key;
        break;
      }
    }
  ...
  if (matchedGenre) {
    const genreData = await getTrendingTracksByGenre(matchedGenre);
    if (onStream) {
      onStream({
        type: "status",
        data: `正在聚合 ${genreData.genreName} 当前热门榜单，并在网易云匹配 320k 音频...`,
      });
    }
    ...
    const cardPayload = {
      title: `${genreData.icon} [DJ Radar] ${genreData.genreName} 趋势热单`,
